...
public class JPAPostRepository implements PostRepository {
    ...
    //List all the posts belong to paticular author
    @Override
    public CompletionStage<Stream<PostData>> listAuthorPosts(String name) {
        return supplyAsync(() -> wrap(em -> selectAuthor(em, name)), ec);
    }
    ...
    //Create query to retrieve corresponding posts
    private Stream<PostData> selectAuthor(EntityManager em, String name) {
        TypedQuery<PostData> query = em.createQuery("SELECT p FROM PostData p WHERE p.author LIKE :authorName", PostData.class).setParameter("authorName", name);
