...
public class PostResourceHandler {
    ...
    public CompletionStage<PostResource> create(PostResource resource) {
        final PostData data = new PostData(resource.getTitle(), resource.getBody(), resource.getAuthor());
    ...
    public CompletionStage<Stream<PostResource>> lookupAuthor(String name) {
        return repository.listAuthorPosts(name).thenApplyAsync(postDataStream -> {
            return postDataStream.map(data -> new PostResource(data, link(data)));
        }, ec.current());
    }
